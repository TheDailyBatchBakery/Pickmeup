...
DROP POLICY IF EXISTS "Orders are viewable by everyone via API" ON orders;
CREATE POLICY "Orders are viewable by everyone via API"
  ON orders FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Orders are insertable by everyone via API" ON orders;
CREATE POLICY "Orders are insertable by everyone via API"
  ON orders FOR INSERT
  WITH CHECK (true);

DROP POLICY IF EXISTS "Orders are updatable by everyone via API" ON orders;
CREATE POLICY "Orders are updatable by everyone via API"
  ON orders FOR UPDATE
  USING (true);

DROP POLICY IF EXISTS "Order items are viewable by everyone via API" ON order_items;
CREATE POLICY "Order items are viewable by everyone via API"
  ON order_items FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Order items are insertable by everyone via API" ON order_items;
